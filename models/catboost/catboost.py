...
class CatBoost_model:
    def __init__(
        ...
        if params is not None:
            if use_gpu:
                ...
                self.cbr = CatBoostRegressor(
                    loss_function=loss_function, verbose=100, **params,
                )
        else:
            if use_gpu:
                self.cbr = CatBoostRegressor(
                ...
                self.cbr = CatBoostRegressor(
                    loss_function=loss_function, verbose=100,
                )
        if multi_target:
    ...
    def train_union(
        ...
        self.cbr.fit(
            X_train[:, -self.train_horizon_union :, :].reshape(
                (len(X_train), -1),
            ),
            y_train[:, 0, :].reshape((len(y_train), -1)),
            eval_set=(
                X_val[:, -self.train_horizon_union :, :].reshape(
                    (len(X_val), -1),
                ),
                y_val[:, 0, :].reshape((len(y_val), -1)),
            ),
        )

    def train_multi(
            self, X_train, y_train, train_horizon: int, pred_horizon: int,
    ):
        self.train_horizon_multi = train_horizon
        self.pred_horizon_multi = pred_horizon
        self.cbr.fit(
            X_train[:, -self.train_horizon_multi :, :].reshape(
                len(X_train), -1,
            ),
            y_train[:, : self.pred_horizon_multi, :].reshape(len(y_train), -1),
        )

    def pred_union(self, X_data, y_data, horizon_pred_out: int):
        assert horizon_pred_out >= 1
        output = [
            self.cbr.predict(
                X_data[:, -self.train_horizon_union :, :].reshape(
                    len(X_data), -1,
                ),
            ),
        ]
        ...
        for i in range(1, horizon_pred_out):
            ...
            if X_data.shape[1] > i:
                ...
                output.append(
                    self.cbr.predict(additional_X[:, :i, :]).reshape(
                        len(X_data), -1,
                    ),
                )
        # FIXME
        ...
        # raise
        output = list(
            map(lambda x: x[::-1], np.array(output).reshape(5000, -1)),
        )
        return (
    ...
    def pred_multi(self, X_pred):
        # FIXME
        return self.predic(
            X_pred[:, -self.train_horizon_multi :, :].reshape(
                len(X_train), -1,
            ),
        )
